...
export async function action({ request }: { request: Request }) {
  ...
  // Mirrors body("metadata").isObject().notEmpty(): both validators ran, so a
  // missing metadata key produced two identical entries while a non-object but
  // non-empty value produced one. An empty object `{}` passes both and is left
  // to AJV, which reports the missing required fields.

...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const schemas = await getAvailableSchemas().catch(() => ({} as Record<string, string[]>));
...
function getDefs(schema: JsonSchema): Record<string, JsonSchema> {
  return ((schema["$defs"] ?? schema["definitions"] ?? {}) as Record<string, JsonSchema>);
}
  ...
  if (type === "array") {
    ...
    if (items) {
      ...
      if (itemRef) {
        ...
        if (resolved && isObjectDef(resolved)) return `${defNameFromRef(itemRef)}[]`;
      }
    }
    return "Array";
  }
  ...
  function visit(name: string, node: JsonSchema, depth: number) {
    ...
    for (const [propName, propSchema] of Object.entries(properties)) {
      ...
      if ((propSchema as JsonSchema)["type"] === "array") {
        const items = (propSchema as JsonSchema)["items"] as JsonSchema | undefined;
        if (items) {
          const itemRef = extractRef(items);
          if (itemRef) {
...
export default function SchemaViewPage() {
  ...
  const mermaidCode =
    schemaData?.schema ? buildClassDiagram(schemaData.schema) : "";
    ...
    <Box sx={{ p: 3 }}>
        ...
        <Typography variant="h5" sx={{ fontWeight: 700, flex: 1 }}>
          ...
          {schemaData && (
            <Typography component="span" variant="body2" color="text.secondary" sx={{ ml: 1.5 }}>
              {schemaData.name} {schemaData.version}
            </Typography>
          )}
        </Typography>
      ...
      {svg && (
        <Typography variant="caption" color="text.secondary" sx={{ mb: 2, display: "block" }}>
          + required &nbsp;·&nbsp; (no marker) optional &nbsp;·&nbsp; depth limit 4, max 25 classes
        </Typography>
      )}
        ...
        <Paper variant="outlined" sx={{ overflow: "hidden" }}>
          <Box
            sx={{
              ...
              py: 0.75,
              cursor: "pointer",
              borderBottom: rawOpen ? "1px solid rgba(255,255,255,0.08)" : "none",
            }}

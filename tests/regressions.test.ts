...
/**
 * Pins the JSON error-response contracts (shapes, status codes) that API
 * consumers such as Gateway-web depend on.
 */
...
describe("400 error-shape parity (missing/invalid params)", () => {
  ...
  it("/get/map unroutable pair → 200 with null map and null path", async () => {
    const res = await fetch(
      `${BASE_URL}/get/map?input_schema=HDRUK&input_version=9.9.9&output_schema=GWDM&output_version=9.9.9`
    );
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.translation_map).toBeNull();
    expect(body.translation_path).toBeNull();
    expect(body.translation_maps).toBeNull();
    expect(body.input_schema).toBe("HDRUK");
    expect(body.output_version).toBe("9.9.9");
  });
